...
(deftest text-mouseup
   (let [act (:actions *driver*)]
      (.moveToElement act (:webelement (element {:tag :canvas})) 25 120)
      (.click act)
      (.perform act))
...
(deftest text-mousemove
   (let [act (:actions *driver*)]
      (.moveToElement act (:webelement (element {:tag :canvas})) 75 120)
      (.perform act))
   (is (= (test-output) "text-mousemove")))

(deftest text-mousedown
   (let [act (:actions *driver*)]
      (.moveToElement act (:webelement (element {:tag :canvas})) 125 120)
      (.click act)
      (.perform act))
