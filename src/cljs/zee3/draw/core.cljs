...
(defn stage [config]
     (let [ {:keys [id width height]} config] 
     	    (let [e (.createElement js/document "canvas")]
     	    	 (set! (.-id e) id)
	  	 (set! (.-width e) width)
	  	 (set! (.-height e) height)
	  	 (.appendChild (aget (.getElementsByTagName js/document "body") 0) e)
		 (setupEvents e)
		 (swap! entities conj (assoc config :type "stage" :ref e))
		 (let [ctx (.getContext e "2d")]
	  	      (swap! contexts assoc id ctx)
		       ctx))))
...
(defn handle-mouse-up [event]
      (let [x (.-x event) y (.-y event)]
      	(let [k (filter #(and (not= "stage" (get % :type)) (intersects % {:x x :y y}) (contains? % :mouseup)) @entities)]
		(doseq [{:keys [mouseup]} k] (mouseup)))))

(defn mouse-down [entities, point] 
	(let [k (filter #(and (intersects % point) (contains? % :mouse-down)) @entities)]
...
(defn absolute-point [stage point]
      ...
      (let [{:keys [ref]} (first (filter #(and (= stage (get % :id)) (= "stage" (get % :type))) @entities)) {:keys [x y]} point]
      (.log js/console ref)
...
(defn get-entity [id]
      (first (filter #(= id (get % :id)) @entities)))